...
---
tout d'abord il faut installer les 3 packages (surtout shiny, car on l'a besoin pour créer l'application web interactive)
...
# vu que le data set est en anglais, on voulais tout traduire en francais
...
ui <- fluidPage(
  ...
  sidebarLayout(
    ...
    mainPanel(
      ...
      # Zone de message d'erreur personnalisée si rien n'est trouvé
...
# Traductions (on doit transformer le choix francais en anglais, valeur du dataset)
    ...
    # Ajout de la colonne adéquation proprement
    ...
    showNotification("Bon voyage ✈️!", type = "message")
  })

  # AFFICHAGE DU TABLEAU CORRIGÉ (Sans colonnes forcées)
  output$resultats <- renderTable({
    req(valeurs$top_voyages)
    # On affiche tout le tableau sélectionné sans nommer les colonnes à la main
    # Cela évite l'erreur "undefined columns"
    valeurs$top_voyages
  ...
  output$prio_ui <- renderUI({
    ...
    tagList(
      hr(),
      h4("⚙️ Pas satisfait ? Imposez 2 critères prioritaires"),
      fluidRow(
        column(6, selectInput("crit1", "Priorité 1", choices = list("Type de voyage"=1, "accompagnant"=2, "Mois"=3, "Climat"=4, "Fréquentation"=5, "Activité"=6))),
        column(6, selectInput("crit2", "Priorité 2", choices = list("Type de voyage"=1, "accompagnant"=2, "Mois"=3, "Climat"=4, "Fréquentation"=5, "Activité"=6)))
      ),
  ...
  observeEvent(input$prio_btn, {
    ...
    output$prio_res <- renderText({
      if (any(strict)) {
        ...
        paste("🌟 La meilleure destination respectant strictement vos priorités est :", best)
      } else {
